# pip install pandas
# pip install numpy   
# pip install streamlit
# pip install matplotlib

# Bar charts and pie charts in streamlit

# Para executar:
# python -m streamlit run "G:\Meu Drive\Cursos\Python\MachineLearning\0805b.py"
# python -m streamlit run C:\Users\Roger\Documents\GitHub\RogerTangRT\Colab\MachineLearning\0805b.py

import pandas as pd
import numpy as np
import streamlit as st
import matplotlib.pyplot as plt

animals = ['cat','cow','dog','goat']
heigths = [30,150,80,60]
weights = [5,400,40,50]

fig, ax = plt.subplots()

x = np.arange(len(heigths))
print('len(heigths) =', len(heigths))
print('x =',x)
width = 0.40

ax.bar(x-0.2,heigths,width, color='red')
ax.bar(x+0.2,weights,width, color='orange')

ax.legend(['heigths','weights'])
ax.set_xticks(x)
ax.set_xticklabels(animals)

st.pyplot(fig)

# Gráfico de Pizza
explode = [0.2,0.1,0.1,0.1]
plot_pie , ax = plt.subplots()
ax.pie(heigths,explode=explode, labels=animals, autopct='%1.1f%%', shadow=True)
ax.axis('equal')
st.pyplot(plot_pie)